...
$resultGroups = mysqli_query($connection, "SELECT id, denumire FROM `carti`");
...
<div class="row">
  <form action="" method="post" enctype="multipart/form-data">
    ...
    <div class="form-group">
      <label for="carte">Book</label>
      <select class="form-control" id="carte" name="carte">
        <option value="0">Select group</option>
          <? while($element = mysqli_fetch_assoc($resultGroups)){?>
        <option value="<?=$element['id'];?>"><?=$element['denumire'];?></option>
        <? }?>
      </select>
